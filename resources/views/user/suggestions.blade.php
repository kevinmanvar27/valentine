                ...
                @foreach($suggestions as $index => $suggestion)
                    <div class="bg-gradient-to-br from-white to-pink-50 rounded-3xl shadow-xl overflow-hidden card-hover border-2 border-valentine-100 hover:border-valentine-300 animate-fade-in" style="animation-delay: {{ $index * 0.1 }}s;">
                        ...
                        <div class="relative h-72 overflow-hidden">
                            @if($suggestion->photo)
                                <img src="{{ Storage::url($suggestion->photo) }}" alt="{{ $suggestion->full_name }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                            @else
                            ...
                            <div class="absolute top-4 left-4 flex gap-2">
                                ...
                                @endif
                                @if($suggestion->created_at->diffInDays() < 3)
                            ...
                            <div class="absolute bottom-4 left-4 right-4 text-white">
                                <h3 class="text-xl font-bold mb-1">{{ $suggestion->full_name }}, {{ $suggestion->age }}</h3>
                                <p class="text-white/90 text-sm flex items-center">
                                    <i class="fas fa-graduation-cap mr-2"></i>
                                    {{ $suggestion->branch }} • {{ $suggestion->year }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="p-6">
                            @if($suggestion->bio)
                                <p class="text-gray-600 text-sm mb-4 line-clamp-2">{{ $suggestion->bio }}</p>
                            @else
                            ...
                            <div class="flex flex-wrap gap-2 mb-4">
                                <span class="bg-valentine-50 text-valentine-600 px-3 py-1 rounded-lg text-xs font-medium">
                                    {{ ucfirst($suggestion->gender) }}
                                </span>
                                @if($suggestion->instagram_id)
                            ...
                            <div class="flex gap-3">
                                ...
                                <button onclick="viewProfile({{ $suggestion->id }})" class="w-12 h-12 bg-gray-100 text-gray-600 rounded-xl flex items-center justify-center hover:bg-gray-200 transition-all duration-300">
